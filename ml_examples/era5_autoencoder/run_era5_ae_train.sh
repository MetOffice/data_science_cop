...
python train_era5_autoencoder.py --config-path config.json --model-out-dir  /gws/ssde/j25a/mmh_storage/user/shaddad/experiments/era5_autoencoder --batch-size=${BATCH_SIZE} --num-epochs ${NUM_EPOCHS} --learning-rate ${LEARNING_RATE} --data-dir $WEATHERBENCH_NORM_DIR  # --mlflow-url "http://localhost" --mlflow-port ${MLFLOW_PORT}
